...
export default function Editor() {
  ...
  const findNodeAtOffset = (root: Node, targetOffset: number): { node: Node, offset: number } | null => {
    let currentOffset = 0
  ...
  const toggleFormatting = (format: 'bold' | 'italic' | 'underline') => {
    ...
    const overlappingRanges = styledRanges.filter(range => 
      range.start <= end && range.end >= start
    )

    const newStyledRanges = styledRanges.filter(range =>
      !(range.start <= end && range.end >= start)
    )
